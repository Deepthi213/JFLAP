...
import automata.event.AutomataTextLabelListener;
...
public class Automaton implements Serializable, Cloneable {
	...
	public Automaton() {
		...
		textLabels = new HashSet<TextLabel>();
		finalTextLabels = new HashSet<TextLabel>();
		initialState = null;
	}
...
//				a.addTransition(ts[i].copy(from, to));
		...
		HashMap<TextLabel, TextLabel> mapTextLabel = new HashMap<>(); 
		Iterator<TextLabel> tl = textLabels.iterator();
		while (tl.hasNext()) {			
			TextLabel textLabel = (TextLabel) tl.next();
			TextLabel ntextLabel = new TextLabel(textLabel.getID(), new Point(textLabel.getPoint()), a);
//				copyRelevantDataForBlocks(nTextboxLabel, TextboxLabel, a);
//				ntextLabel.setLabel(textLabel.getLabel());
			ntextLabel.setName(textLabel.getName());
			mapTextLabel.put(textLabel, ntextLabel);
			a.addTextLabel(ntextLabel);
			System.out.println(textLabel.getName() + " - m ");
			/*
			 * If it is a Moore machine, set the TextboxLabel output.
			 */
			/*
			 * if (this instanceof MooreMachine) { MooreMachine m = (MooreMachine) a;
			 * m.setOutput(nTextboxLabel, ((MooreMachine) this).getOutput(TextboxLabel)); }
			 */
		}
		

		tl = finalTextLabels.iterator();
		while (tl.hasNext()) {
			TextLabel textLabel = (TextLabel) tl.next();
			a.addFinalTextLabel((TextLabel) mapTextLabel.get(textLabel));
		}
		a.setInitialTextLabel((TextLabel) mapTextLabel.get(getInitialTextLabel()));
...
//				dest.addTransition(ts[i].copy(from, to));
		...
		Iterator<TextLabel> tl = src.textLabels.iterator();
		while (tl.hasNext()) {
			TextLabel curTextLabel = (TextLabel) tl.next();			
			dest.addTextLabel(new TextLabel(curTextLabel.getID(), new Point(curTextLabel.getPoint()), dest));
		}
	...
	public TextLabel createTextLabel(Point point) {
		int i = 0;
		while (getTextLabelWithID(i) != null)
			i++;
		TextLabel textLabel = new TextLabel(i, point, this);
	...
	/**
	 * Creates a state, inserts it in this automaton, and returns that state. The ID
	 * for the state is set appropriately.
	 * 
	 * @param point the point to put the state at
	 */
	public final TextLabel createTextLabelWithId(Point point, int i) {
		TextLabel textLabel = new TextLabel(i, point, this);
		addTextLabel(textLabel);
		return textLabel;
	}
	...
	public void removeTextLabel(TextLabel textLabel) {
		...
		distributeTextLabelEvent(new AutomataTextLabelEvent(this, textLabel, false, false, false));
		textLabels.remove(textLabel);
		finalTextLabels.remove(textLabel);
		if (textLabel == initialTextLabel)
			initialTextLabel = null;

		cachedTextLabels = null;
...
//		}
	...
	public TextLabel setInitialTextLabel(TextLabel initialTextLabel) {
		TextLabel oldInitialTextLabel = this.initialTextLabel;
		this.initialTextLabel = initialTextLabel;
		distributeTextLabelEvent(new AutomataTextLabelEvent(this, initialTextLabel, false, false, true));
		return oldInitialTextLabel;
	}
	 ...
	 * @return the start state for this automaton
	 */
	public TextLabel getInitialTextLabel() {
		return this.initialTextLabel;
	}
	...
	public void selectTextLabelsWithinBounds(Rectangle bounds) {
		TextLabel[] textLabels = getTextLabels();
		System.out.println(textLabels.length);
		for (int k = 0; k < textLabels.length; k++) {
			textLabels[k].setSelect(false);
			if (bounds.contains(textLabels[k].getPoint())) {
				textLabels[k].setSelect(true);
			}
		}
	}
	...
	public void addTextLabel(TextLabel textLabel) {
		...
		cachedTextLabels=null;
		distributeTextLabelEvent(new AutomataTextLabelEvent(this, textLabel, true, false, false));
	}

	/**
	 * Returns an array that contains every state in this automaton. The array is
	 * gauranteed to be in order of ascending state IDs.
	 * 
	 * @return an array containing all the states in this automaton
	 */
	public TextLabel[] getTextLabels() {
		if (cachedTextLabels == null) 
		{
			cachedTextLabels = (TextLabel[]) textLabels.toArray(new TextLabel[0]);
			System.out.println("cachedTextLabels[0] : " + textLabels.toArray().length);
			Arrays.sort(cachedTextLabels, new Comparator<Object>() {
				public int compare(Object o1, Object o2) {
					return ((TextLabel) o1).getID() - ((TextLabel) o2).getID();
				}
		...
		System.out.println("cachedTextLabels.length : " + cachedTextLabels.length);
	...
	public void addFinalTextLabel(TextLabel finalTextLabel) {
		cachedFinalTextLabels = null;
		finalTextLabels.add(finalTextLabel);
		distributeTextLabelEvent(new AutomataTextLabelEvent(this, finalTextLabel, false, false, true));
	}
	...
	public boolean isFinalTextLabel(TextLabel textLabel) {
		return finalTextLabels.contains(textLabel);
	}

	/**
	 * Determines if the state passed in is the initial states. Added for JFLAP 6.3
	 * 
	 * @param state the state to determine if is final
	 * @return <CODE>true</CODE> if the state is a final state in this automaton,
	 *         <CODE>false</CODE> if it is not
	 */
	public boolean isInitialTextLabel(TextLabel textLabel) {
		return (textLabel.equals(initialTextLabel));
	}
	/**
	...
	public TextLabel getTextLabelWithID(int id) {
		Iterator<TextLabel> it = textLabels.iterator();
		while (it.hasNext()) {
			TextLabel textLabel = (TextLabel) it.next();
	...
	void distributeTextLabelEvent(AutomataTextLabelEvent event) {
		Iterator<AutomataTextLabelListener> it = textLabelListeners.iterator();
		while (it.hasNext()) {
			AutomataTextLabelListener listener = (AutomataTextLabelListener) it.next();
			listener.automataTextLabelChange(event);
		}
	}
...
//        EDebug.print("The Hash is that is hashed, is truly hashed");
		...
		for (Object o : textLabels)
			ret += ((TextLabel) o).specialHash();
		ret += finalStates.hashCode();
		ret += finalTextLabels.hashCode();
		ret += initialState == null ? 0 : (int) (initialState.specialHash() * Math.PI);
		ret += initialTextLabel == null ? 0 : (int) (initialTextLabel.specialHash() * Math.PI);
//        EDebug.print(ret);
		return ret;
	}
	...
	public Set<TextLabel> textLabels;

	/** The cached array of states. */
	private State[] cachedStates = null;

	private TextLabel[] cachedTextLabels = null;
	...
	private TextLabel[] cachedFinalTextLabels = null;
	/**
	...
	public Set<TextLabel> finalTextLabels;

	/** The initial state. */
	protected State initialState = null;

	protected TextLabel initialTextLabel = null;
...
//	private HashMap blockMap = new HashMap();
	...
	private ArrayList<TextLabel> myTextLabels = new ArrayList<TextLabel>();
	...
	private transient HashSet<AutomataTextLabelListener> textLabelListeners = new HashSet<AutomataTextLabelListener>();
    ...
    public void clear(){
		...
		t = new HashSet<Object>(textLabels);
		for (Object o : t)
			removeTextLabel((TextLabel) o);
		textLabels = new HashSet<TextLabel>();
		myTextLabels = new ArrayList<TextLabel>();
		...
		while (myTextLabels.size() != 0) {
			AutomatonPane ap = ((TextLabel) myTextLabels.get(0)).getView();
			ap.remove((TextLabel) myTextLabels.get(0));
			ap.repaint();
			removeTextLabel((TextLabel) myTextLabels.get(0));
		}
