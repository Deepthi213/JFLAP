...
public abstract class AutomatonTransducer extends AbstractTransducer {
	...
	protected Map<Integer, TextLabel> readTextLabels(Node node, Automaton automaton, Set<Object> locatedTextLabels,
			Document document) {
		Map<Integer, TextLabel> i2s = new java.util.HashMap<Integer, TextLabel>();
		if (node == null)
			return i2s;
		NodeList allNodes = node.getChildNodes();
		ArrayList<Node> textLabelNodes = new ArrayList<Node>();
		for (int k = 0; k < allNodes.getLength(); k++) {
			if (allNodes.item(k).getNodeName().equals(TEXTLABEL_NAME)) {
				textLabelNodes.add(allNodes.item(k));
			}
		}
		// Map state IDs to states, in an attempt to add in numeric
		// things first. A specialized Comparator is helpful here.
		...
		createTextLabel(textLabelNodes, i2sn, automaton, locatedTextLabels, i2s, false, document);
		// i2s = addBlocks(document, automaton, locatedStates, i2s);
		return i2s;
	}

	protected void createTextLabel(ArrayList<Node> TextLabelNodes, Map<Integer, Node> i2sn, Automaton automaton,
			Set<Object> locatedTextLables, Map<Integer, TextLabel> i2s, boolean isBlock, Document document) {
		// Create the map of ids to state nodes.
		for (int i = 0; i < TextLabelNodes.size(); i++) {
			Node TextLabelNode = (Node) TextLabelNodes.get(i);
			if (TextLabelNode.getNodeType() != Node.ELEMENT_NODE)
				continue;
			// Extract the ID attribute.
			String idString = ((Element) TextLabelNode).getAttribute(TEXTLABEL_ID_NAME);
			...
			if (i2sn.put(id, TextLabelNode) != null)
		...
		while (it.hasNext()) {
			...
			Element textLabelNode = (Element) i2sn.get(id);
			// Get the fields of this state.
			Map<String, String> e2t = elementsToText(textLabelNode);
			// Create the state.
			...
			double x = 0, y = 0;
			try {
				x = Double.parseDouble(e2t.get(TEXTLABEL_X_COORD_NAME).toString());
			} catch (NullPointerException e) {
				hasLocation = false;
			} catch (NumberFormatException e) {
				throw new DataException("The x coordinate " + e2t.get(TEXTLABEL_X_COORD_NAME)
						+ " could not be read for state " + id + ".");
			}
			// Try to get the Y coord.
			try {
				y = Double.parseDouble(e2t.get(TEXTLABEL_Y_COORD_NAME).toString());
			} catch (NullPointerException e) {
				hasLocation = false;
			} catch (NumberFormatException e) {
				throw new DataException("The y coordinate " + e2t.get(TEXTLABEL_Y_COORD_NAME)
						+ " could not be read for state " + id + ".");
			}
			p.setLocation(x, y);
			// Create the state.
...
//			if (!isBlock){
			if (!(automaton instanceof TuringMachine)) {
				textlabel = automaton.createTextLabelWithId(p, id.intValue());
			} else {
				Node tempNode = null;
...
//                    EDebug.print("Are we or not creating a block?");
					// textlabel = ((TuringMachine) automaton).createInnerTM(p, temp, fileName,
					// id.intValue());
				} else {
					// textlabel = ((TuringMachine) automaton).createTMStateWithID(p,
					// id.intValue());
				}
			}
			if (hasLocation && locatedTextLables != null)
				locatedTextLables.add(textlabel);
			i2s.put(id, textlabel);
			String name = ((Element) textLabelNode).getAttribute(TEXTLABEL_NAME_NAME);
			if (name.equals(""))
				textlabel.setName("q" + id.intValue());
			else
				textlabel.setName(name);

			// Add various attributes.
			if (e2t.containsKey(TEXTLABEL_NAME_NAME))
				textlabel.setName((String) e2t.get(TEXTLABEL_NAME_NAME));

			if (e2t.containsKey(STATE_FINAL_NAME))
				automaton.addFinalTextLabel(textlabel);
			if (e2t.containsKey(STATE_INITIAL_NAME))
				automaton.setInitialTextLabel(textlabel);
			/*
			 * If it is a Moore machine, add state output.
			 */
			if (automaton instanceof MooreMachine && e2t.containsKey(MooreTransducer.TEXTLABEL_OUTPUT_NAME))
				((MooreMachine) automaton).setOutput(textlabel,
						(String) e2t.get(MooreTransducer.TEXTLABEL_OUTPUT_NAME));
		}
	}
...
//                    EDebug.print("Are we or not creating a block?");
	...
	public java.io.Serializable readAutomaton(Node parent, Document document) {
		...
		Set<Object> locatedTextLabels = new java.util.HashSet<>();
		...
		readTransitions(parent, root, readStates(parent, root, locatedStates,
				document));
		readTextLabels(parent, root, locatedTextLabels, document);
		//read the notes
...
//		}
	...
	protected Element createTextLabelElement(Document document, TextLabel textlabel, Automaton container) {
		// Start the creation of the state tag.
		Element se = createElement(document, TEXTLABEL_NAME, null, null);
		se.setAttribute(TEXTLABEL_ID_NAME, "" + textlabel.getID());
		// Encode position.
		se.appendChild(createElement(document, TEXTLABEL_X_COORD_NAME, null, "" + textlabel.getPoint().getX()));
		se.appendChild(createElement(document, TEXTLABEL_Y_COORD_NAME, null, "" + textlabel.getPoint().getY()));

		// Encode the name, if set.
		if (textlabel.getName() != null)
			se.setAttribute(TEXTLABEL_NAME_NAME, "" + textlabel.getName());
...
//		}
	...
	private Element writeFields(Document doc, Automaton auto, Element se) {
		...
		TextLabel[] textlabels = auto.getTextLabels();
		if (textlabels.length > 0)
			se.appendChild(createComment(doc, COMMENT_TEXTLABELS));
		for (int i = 0; i < textlabels.length; i++)
			se.appendChild(createTextLabelElement(doc, textlabels[i], auto));
...
//		 Start the creation of the transition.
	...
	/** The tag name for individual TEXTLABEL elements. */
	public static final String TEXTLABEL_NAME = "textlabel";

	/** The attribute name for the TEXTLABEL ID. */
	public static final String TEXTLABEL_ID_NAME = "id";

	/** The tag name for the X coordinate. */
	public static final String TEXTLABEL_X_COORD_NAME = "x";

	/** The tag name for the Y coordinate. */
	public static final String TEXTLABEL_Y_COORD_NAME = "y";
	...
	/** The tag name for the optional special name of the TEXTLABEL. */
	public static final String TEXTLABEL_NAME_NAME = "name";
	...
	private static final String COMMENT_TEXTLABELS = "The list of textLabels.";	
