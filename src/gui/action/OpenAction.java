...
public class OpenAction extends RestrictedAction {
	 ...
	 */
	public OpenAction(NewAction newAction) {
		super("Open...", null);
		putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_O, MAIN_MENU_MASK));
		putValue(SHORT_DESCRIPTION, "Open tool short description.......");
	...
	/**
	 * If an open is attempted, call the methods that handle the retrieving of an
	 * object, then create a new frame for the environment.
	 * 
	...
	public void actionPerformed(ActionEvent event) {
		...
		for (int k = 0; k < files.length; k++) {
			...
			try {
				openFile(file, codecs, this.newaction);
	...
	public static java.io.Serializable readFileAndCodecs(File file, NewAction newAction) {
		OpenAction.setOpenOrRead(true);
		Codec[] codecs = null;
		codecs = makeFilters();
		openFile(file, codecs, newAction);
	 ...
	 */
	public static void openFile(File file, Codec[] codecs, NewAction newAction) {
		dontOpen = false;
		...
		for (int i = 0; i < codecs.length; i++) {
			try {
				...
				if (!openOrRead) {
					if (newAction != null)
						newAction.OpenFile(object, file, codecs[i]);
					/*
					 * EnvironmentFrame ef = FrameFactory.createFrame(object); if (ef == null)
					 * return; ef.getEnvironment().setFile(file); ef.getEnvironment().setEncoder(
	...
	/**
	 * Set to true when user attempts to open incorrect file type as a building
	 * block
	 */
