...
public class ArrowTool extends Tool {
	...
	public void mouseClicked(MouseEvent event) {
		...
		lastClickedTextLabel = getDrawer().TextLabelAtPoint(event.getPoint());
		System.out.println(
				"getDrawer().TextLabelAtPoint(event.getPoint()) : " + getDrawer().TextLabelAtPoint(event.getPoint()));
		if (getDrawer().TextLabelAtPoint(event.getPoint()) == null) {
...
//				this.setText("helllp");
				...
				String oldlabel = lastClickedTextLabel.getName();
				oldlabel = oldlabel == null ? "" : oldlabel;

				String label = (String) JOptionPane
						.showInputDialog("Input a new label, or \n" + "set blank to remove the label", oldlabel);
				if (label == null)
					return;
				if (label.equals(""))
					label = null;
				lastClickedTextLabel.setName(label);

				getView().repaint();

				System.out.println("label : " + lastClickedTextLabel.getName());
			}
		}
	}
	...
	/**
	 * On a mouse press, allows the state to be dragged about unless this is a popup
	 * trigger.
	 */
	public void mousePressed(MouseEvent event) {
		...
		if (lastClickedState != null) {
			...
			initialPointClick.setLocation(event.getPoint());
		} else if (lastClickedTextLabel != null) {
			initialPointTextlabel.setLocation(lastClickedTextLabel.getPoint());
			if (!lastClickedTextLabel.isSelected()) {
				Rectangle bounds = new Rectangle(0, 0, -1, -1);
				getView().getDrawer().getAutomaton().selectTextLabelsWithinBounds(bounds);
				getView().getDrawer().setSelectionBounds(bounds);
				lastClickedTextLabel.setSelect(true);
			}
			getView().repaint();
		} else {
	 ...
	 *         <CODE>false</CODE> if it should not be... returns <CODE>true</CODE>
	 *         by default
	 */
	...
	public void mouseDragged(MouseEvent event) {
		...
		if (lastClickedState != null) {
			...
			TextLabel[] textLabels = getView().getDrawer().getAutomaton().getTextLabels();
			for (int k = 0; k < textLabels.length; k++) {
				TextLabel curTextLabel = textLabels[k];
				if (curTextLabel.isSelected()) {
					int x = curTextLabel.getPoint().x + p.x - initialPointClick.x;
					int y = curTextLabel.getPoint().y + p.y - initialPointClick.y;
					curTextLabel.getPoint().setLocation(x, y);
					curTextLabel.setPoint(curTextLabel.getPoint());
				}
			}
			initialPointClick = p;
			getView().repaint();
			...
			if (f == t) {
				...
				/*
				 * double circlex = (p.x-f.getPoint().x); double circley = (p.y-f.getPoint().y);
				 * double angle = Math.atan2(circley, circlex); Point from =
				 * getView().getDrawer().pointOnState(f, angle+Math.PI*.166); Point to =
				 * getView().getDrawer().pointOnState(f, angle-Math.PI*.166);
				 * 
				 * Transition[] trans = getAutomaton().getTransitionsFromStateToState(f, t);
				 * 
				 * for (int n = 0; n < trans.length; n++) { CurvedArrow arrow = (CurvedArrow)
			...
			if (f != t) {
				...
				for (int n = 0; n < trans.length; n++) {
					...
					// uncomment this code for Transitions movement
					/*
					 * float centerx = (t.getPoint().x+f.getPoint().x)/2; float centery =
					 * (t.getPoint().y+f.getPoint().y)/2; float pvecx = p.x-centerx; float pvecy =
					 * p.y-centery; float svecx = t.getPoint().x-centerx; float svecy =
					 * t.getPoint().y-centery; float dprod = pvecx*svecx+pvecy*svecy; dprod =
					 * dprod/(float) Math.abs((Math.sqrt(pvecx*pvecx+pvecy*pvecy)))/(float)
					 * Math.abs((Math.sqrt(svecx*svecx+svecy*svecy))); float theta = (float)
					 * Math.acos(dprod); float curv = (float)
					...
					// curve.setCurve(curve.getX1(), curve.getY1(), p.x, p.y, curve.getX2(),
					// curve.getY2());
	...
	public void mouseReleased(MouseEvent event) {
		...
		if (lastClickedState != null) {
			...
			EDebug.print("event.isPopupTrigger() 1- " + event.isPopupTrigger());
			if (event.isPopupTrigger())
				showPopup(event);
			...
			int count = 0;
			for (int k = 0; k < textLabels.length; k++) {
				if (textLabels[k].isSelected()) {
					count++;
				}
			}
			Rectangle bounds = getView().getDrawer().getSelectionBounds();
			if (count == 1 && bounds.isEmpty() && lastClickedTextLabel != null)
				lastClickedTextLabel.setSelect(false);
			bounds = new Rectangle(0, 0, -1, -1);
			getView().getDrawer().setSelectionBounds(bounds);
			lastClickedTextLabel = null;
			getView().repaint();
		}
	}
	...
	protected class StateMenu extends JPopupMenu implements ActionListener {
		...
		public void show(State state, Component comp, Point at) {
			...
			if (getDrawer().getAutomaton() instanceof TuringMachineBuildingBlocks) { // add building block options for
																						// TMBB
				// System.out.println(getDrawer().getAutomaton().getEnvironmentFrame());
				// System.out.println(getDrawer().getAutomaton().getEnvironmentFrame().getEnvironment());
				this.add(editBlock);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (item == makeFinal) {
			...
			} else if (item == editBlock) { // this implies that this was a TMState to begin with, because only TM
											// states would have this menu option
				...
				// pop up box asking for building block name if myInternalName has not already
				// been set or
				...
				if (tmState.myInternalName == null || tmState.myInternalName.contains("Machine" + tmState.getID())) {
					...
					int result = JOptionPane.showOptionDialog((Component) e.getSource(), panel,
							"Give Building Block a Name", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE,
							null, null, null);
					...
					for (State regState : tmState.getAutomaton().states) {
						...
						if (stateTM.getInternalName().equals(input + ".jff")) {
							...
							int selectedOption = JOptionPane.showOptionDialog((Component) e.getSource(),
									"We STRONGLY suggest to NOT "
											+ "use building blocks with the same name. Do you wish to continue anyways?",
									"Same Name as Another Building Block", JOptionPane.DEFAULT_OPTION,
				...
				EditBlockPane editor = new EditBlockPane(((TMState) state).getInnerTM()); // give it a Turing Machine
																							// //just edit the Automaton
																							// directly; there is no
																							// need for a repaint
																							// either, because the other
																							// guy does not paint it
...
//				TMState buffer = ((TuringMachine) getAutomaton()).createTMState((Point)state.getPoint()); //again, we assume that the cast will work, since copyBlock hould never be there except with Turing.
				TMState buffer = ((TuringMachineBuildingBlocks) getAutomaton())
						.createTMState(new Point(state.getPoint().x + 4, state.getPoint().y)); // again, we assume that
																								// the cast will work,
																								// since copyBlock hould
																								// never be there except
																								// with Turing.
				buffer.setInnerTM((TuringMachineBuildingBlocks) ((TMState) state).getInnerTM().clone()); // all states
																											// have an
																											// inner TM,
																											// although
																											// this
		...
		private void replaceCharactersInBlock(TMState start, String toReplace, String replaceWith) { // this shall be a
																										// recursive
																										// method,
																										// replacing the
																										// inside and
		...
		/*
		 * Changed this from private to protected so I can remove "Final State" option
		 * from Moore and Mealy machines.
		 */
...
//          END SJK add
	...
	private TextLabel lastClickedTextLabel = null;
	...
	/** The initial point of the Textlabel. */
	private Point initialPointTextlabel = new Point();
