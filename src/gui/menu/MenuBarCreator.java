 ...
 * application. The new User Interface of JFLAP also specifies the untitled
 * machine that is opened instead of ‘untitled’. JFLAP now specifies JFLAP
 * 7.1(Turing Machine) <untitled’x’> if user selects Turing Machine or JFLAP
 * 7.1(Finite Automaton) <untitled’x’> for an untitled Finite Automaton.
 * 
 * @author Thomas Finley
 */

public class MenuBarCreator {
	public static JMenuBar getMenuBar(EnvironmentFrame frame, NewAction newAction) {
		JMenuBar bar = new JMenuBar();
		JMenu menu;

		menu = getFileMenu(frame, newAction);
	...
	private static JMenu getFileMenu(EnvironmentFrame frame) {
		...
		addItem(menu, (Action) new NewFile());
		SecurityManager sm = System.getSecurityManager();
		if (Universe.CHOOSER != null) {
			// Can't open and save files.
			addItem(menu, new OpenAction(null));
			addItem(menu, new SaveAction(environment, null));
			addItem(menu, new SaveAsAction(environment, null));
			JMenu saveImageMenu;
			...
			if (environment instanceof AutomatonEnvironment) { // this is strictly for non-Grammar
				...
				if (new File("svg.jar").exists() || (jar != null && jar.getJarEntry("org/foo.txt") != null)) {
					...
					try {
						RestrictedAction ra = (RestrictedAction) Class.forName("gui.action.ExportAction")
		...
		addItem(menu, new SubmitToMoodle());
...
//        }
	 ...
	 * @return a file menu
	 */
	private static JMenu getFileMenu(EnvironmentFrame frame, NewAction newAction) {
		Environment environment = frame.getEnvironment();
		JMenu menu = new JMenu("File");
		addItem(menu, (Action) new NewFile());
		SecurityManager sm = System.getSecurityManager();
		if (Universe.CHOOSER != null) {
			// Can't open and save files.
			addItem(menu, new OpenAction(newAction));
			addItem(menu, new SaveAction(environment, newAction));
			addItem(menu, new SaveAsAction(environment, newAction));
			JMenu saveImageMenu;
			saveImageMenu = new JMenu("Save Image As...");
			saveImageMenu.add(new SaveGraphJPGAction(environment, menu));
			saveImageMenu.add(new SaveGraphPNGAction(environment, menu));
...
//        }
	...
	private static JMenu getInputMenu(EnvironmentFrame frame) {
		 ...
		 * GrammarOutputAction ((gui.environment.GrammarEnvironment) environment));
		 */
		...
		if (LLParseTableAction.isApplicable(object))
			addItem(menu, new LLParseTableAction((gui.environment.GrammarEnvironment) environment));
		if (LRParseTableAction.isApplicable(object))
			addItem(menu, new LRParseTableAction((gui.environment.GrammarEnvironment) environment));
		if (BruteParseAction.isApplicable(object))
			addItem(menu, new BruteParseAction((gui.environment.GrammarEnvironment) environment));
		if (MultipleBruteParseAction.isApplicable(object))
			addItem(menu, new MultipleBruteParseAction((gui.environment.GrammarEnvironment) environment));
		if (UserControlParseAction.isApplicable(object))
			addItem(menu, new UserControlParseAction((gui.environment.GrammarEnvironment) environment));
		if (CYKParseAction.isApplicable(object))
			addItem(menu, new CYKParseAction((gui.environment.GrammarEnvironment) environment));
		if (MultipleCYKParseAction.isApplicable(object))
			addItem(menu, new MultipleCYKParseAction((gui.environment.GrammarEnvironment) environment));
...
//					(gui.environment.GrammarEnvironment) environment));
		...
		if (LSystemDisplay.isApplicable(object))
			addItem(menu, new LSystemDisplay((gui.environment.LSystemEnvironment) environment));
	...
	private static JMenu getInputMenu(EnvironmentFrame frame, int specialCaseForTuringConverted) {
		...
		if (UserControlParseAction.isApplicable(object))
			addItem(menu, new UserControlParseAction((gui.environment.GrammarEnvironment) environment));
		if (TuringBruteParseAction.isApplicable(object))
			addItem(menu, new TuringBruteParseAction((gui.environment.GrammarEnvironment) environment));
...
//			menu.add(new StateColorSelector(automaton,environment,menu));
	...
	private static JMenu getTestMenu(EnvironmentFrame frame) {
		...
		if (DFAEqualityAction.isApplicable(object))
			addItem(menu, new DFAEqualityAction((automata.fsa.FiniteStateAutomaton) object, environment));

		/*
		 * if (MinimizeAction.isApplicable(object)) addItem(menu, new MinimizeAction
		 * ((automata.fsa.FiniteStateAutomaton) object, environment));
		 */
		if (NondeterminismAction.isApplicable(object))
			addItem(menu, new NondeterminismAction((automata.Automaton) object, environment));
		/*
		...
		if (LambdaHighlightAction.isApplicable(object))
			addItem(menu, new LambdaHighlightAction((automata.Automaton) object, environment));

		/*
		 * if (GrammarTestAction.isApplicable(object)) addItem(menu, new
		 * GrammarTestAction ((gui.environment.GrammarEnvironment) environment));
		 */
	...
	private static JMenu getConvertMenu(EnvironmentFrame frame) {
		...
		if (NFAToDFAAction.isApplicable(object))
			addItem(menu, new NFAToDFAAction((automata.fsa.FiniteStateAutomaton) object, environment));
		if (MinimizeTreeAction.isApplicable(object))
			addItem(menu, new MinimizeTreeAction((automata.fsa.FiniteStateAutomaton) object, environment));

		if (ConvertFSAToGrammarAction.isApplicable(object))
			addItem(menu, new ConvertFSAToGrammarAction((gui.environment.AutomatonEnvironment) environment));
		if (ConvertPDAToGrammarAction.isApplicable(object))
			addItem(menu, new ConvertPDAToGrammarAction((gui.environment.AutomatonEnvironment) environment));

		if (ConvertFSAToREAction.isApplicable(object))
			addItem(menu, new ConvertFSAToREAction((gui.environment.AutomatonEnvironment) environment));

		if (ConvertCFGLL.isApplicable(object))
			addItem(menu, new ConvertCFGLL((gui.environment.GrammarEnvironment) environment));
		if (ConvertCFGLR.isApplicable(object))
			addItem(menu, new ConvertCFGLR((gui.environment.GrammarEnvironment) environment));
		if (ConvertRegularGrammarToFSA.isApplicable(object))
			addItem(menu, new ConvertRegularGrammarToFSA((gui.environment.GrammarEnvironment) environment));
		if (GrammarTransformAction.isApplicable(object))
			addItem(menu, new GrammarTransformAction((gui.environment.GrammarEnvironment) environment));

		if (REToFSAAction.isApplicable(object))
			addItem(menu, new REToFSAAction((gui.environment.RegularEnvironment) environment));

		if (CombineAutomaton.isApplicable(object))
			addItem(menu, new CombineAutomaton((gui.environment.AutomatonEnvironment) environment));

		if (TuringToUnrestrictGrammarAction.isApplicable(object))
			addItem(menu, new TuringToUnrestrictGrammarAction((gui.environment.AutomatonEnvironment) environment));

		if (AddTrapStateToDFAAction.isApplicable(object))
			addItem(menu, new AddTrapStateToDFAAction((gui.environment.AutomatonEnvironment) environment));
	...
	private static JMenu getHelpMenu(EnvironmentFrame frame) {
		...
		addItem(menu, new AbstractAction("Help...") {
			...
			public void actionPerformed(ActionEvent event) {
				...
				HelpDialog.openHelpWindow(frame);//
