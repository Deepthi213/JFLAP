...
//			drawState(g, hs.get(i));
		...
		TextLabel[] textLabels = automaton.getTextLabels();
		for (int i = 0; i < textLabels.length; i++) {
			drawTextLabel(g, textLabels[i]);
		}
...
//			System.out.println("Using cache");
		...
		TextLabel[] textLabels = getAutomaton().getTextLabels();
		for (int k = 0; k < textLabels.length; k++) {
			TextLabel curNote = ((TextLabel) textLabels[k]);
			Rectangle newBounds = new Rectangle(curNote.getPoint(), new Dimension(curNote.getBounds().getSize()));
			rect.add(newBounds);
		}
...
//		return cachedBounds = rect;
	...
	protected void drawTextLabel(Graphics g) {
		if (textLabels != null) {
			Graphics2D g2 = (Graphics2D) g;
			System.out.println("drawTextLabel : ");
			Iterator<TextLabelArrow> it = textLabels.iterator();
			while (it.hasNext()) {
				TextLabelArrow textLabelArrow = (TextLabelArrow) it.next();
				if (textLabelArrow.myTextLabel.isSelected()) {
					textLabelArrow.drawHighlight(g2);
					textLabelArrow.drawControlPoint(g2);
				} else
					textLabelArrow.draw(g2);
			}
		}
	}

	protected void drawTextLabel(Graphics g, TextLabel textLabel) {
		textlabeldrawer.drawTextLabel(g, getAutomaton(), textLabel);
		/*if (drawLabels) {
			textlabeldrawer.drawTextLabelLabel(g, textLabel, textLabel.getPoint(), StateDrawer.STATE_COLOR);
		}
		Graphics2D g2 = (Graphics2D) g;*/
		/*
		 * System.out.println("drawTextLabel : "+textLabels.size());
		 * Iterator<TextLabelArrow> it = textLabels.iterator(); while (it.hasNext()) {
		 * TextLabelArrow textLabelArrow = (TextLabelArrow) it.next(); if
		 * (textLabelArrow.myTextLabel.isSelected()){ textLabelArrow.drawHighlight(g2);
		 * textLabelArrow.drawControlPoint(g2); } else textLabelArrow.draw(g2); }
		 */
	}
	...
	public TextLabel TextLabelAtPoint(Point point) {
		TextLabel[] textLabels = getAutomaton().getTextLabels();
		// Work backwards, since we want to select the "top" state,
		// and states are drawn forwards so later is on top.
		for (int i = textLabels.length - 1; i >= 0; i--)
			if (point.distance(textLabels[i].getPoint()) <= StateDrawer.STATE_RADIUS)
				return textLabels[i];
		// Not found. Drat!
		return null;
	}
...
//	}
	...
	public Set<TextLabelArrow> textLabels;
	...
	public TextLabelDrawer textlabeldrawer = new TextLabelDrawer();
