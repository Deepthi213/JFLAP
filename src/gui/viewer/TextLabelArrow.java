...
public class TextLabelArrow {
	 ...
	 *              are rather curvy
	 */
	public TextLabelArrow(int x1, int y1, int x2, int y2, float curvy, Transition t) {
	 ...
	 *              are rather curvy
	 */
	public TextLabelArrow(Point start, Point end, float curvy, Transition t) {
		curve = new QuadCurve2D.Float();
		setStart(start);
		setEnd(end);
		control = new Point();
	...
	public void drawText(Graphics2D g) {
		...
		for (int i = 0; i < label.length(); i += CHARS_PER_STEP) {
			...
			g2.drawString(sublabel + "     drawingtext", -dx, dy);
	...
	/**
	 * Draws an arrow head on the graphics object. The arrow geometry is based on
	 * the point of its head as well as another point, which the arrow is defined as
	 * facing away from. This arrow head has no body.
	 * 
	 ...
	 *             along the line segment extending from the head backwards from the
	 *             head if this were an arrow with a line trailing the head
	 */
...
//        System.out.println("Curve refreshing");
	...
	/**
	 * Checks if something is on the line. If it appears to be, then it subdivides
	 * the curve into halves and tries again recursively until the flatness of the
	 * curve is less than the fudge. Frankly, I am a fucking genius. I am one of two
	 * people in this department that could have possibly thought of this.
	 * 
