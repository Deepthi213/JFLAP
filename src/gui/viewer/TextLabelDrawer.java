...
public class TextLabelDrawer {
	...
	/**
	 * Creates a <CODE>StateDrawer</CODE> with states drawn to a particular radius.
	 * 
	 * @param radius the radius of states drawn by this state drawer
	 */
	public TextLabelDrawer(int radius) {
		this.radius = radius;
	}
	 ...
	 */
	public void drawTextLabel(Graphics g, Automaton automaton, TextLabel textLabel) {
		this.drawTextLabel(g, automaton, textLabel, textLabel.getPoint());
	}
	 ...
	 */
	public void drawTextLabel(Graphics g, Automaton automaton, TextLabel textLabel, Point point) {
		drawTextLabel(g, automaton, textLabel, point, STATE_COLOR);
	}

	/**
	 * Draws an individual state at the point specified with the color specified.
	 * 
	 * @param g         the graphics object to draw upon
	 * @param state     the state to draw
	 * @param automaton the automaton this state is a part of
	 ...
	 */
	public void drawTextLabel(Graphics g, Automaton automaton, TextLabel textLabel, Point point, Color color) {
		drawArea(g, automaton, textLabel, point, color);
		return;
	}
	 ...
	 * @param state
	 */
	private void drawArea(Graphics g, Automaton automaton, TextLabel textLabel, Point point, Color color) {
		// Draw the basic background of the state.
		//drawBackground(g, textLabel, point, color);
		// What about the text label?
		g.setColor(Color.black);

		int dx = ((int) g.getFontMetrics().getStringBounds(textLabel.getName(), g).getWidth()) >> 1;
		int dy = ((int) g.getFontMetrics().getAscent()) >> 1;
		System.out.println("textLabel.getName() -# "+textLabel.getName());
		g.drawString(textLabel.getName(), point.x - dx, point.y + dy);
...
//		}
	 ...
	 * @param point the point of the state, which is NOT the same thing as any point
	 *              where the label gets drawn
	 * @param color the background color of the label
	 */
	public void drawTextLabelLabel(Graphics g, TextLabel textLabel, Point point, Color color) {
		/*
		 * String[] labels = textLabel.getLabels(); if (labels.length == 0) return;
		 * 
		 * int ascent = g.getFontMetrics().getAscent(); int heights = 0; int textWidth =
		 * 0;
		 * 
		 * for (int i = 0; i < labels.length; i++) { Rectangle2D bounds =
		 * g.getFontMetrics().getStringBounds(labels[i], g); textWidth = Math.max((int)
		 * bounds.getWidth(), textWidth); heights += ascent + STATE_LABEL_PAD; } heights
		 * -= STATE_LABEL_PAD;
		 * 
		 * // Width of the box. int width = textWidth + (STATE_LABEL_PAD << 1); int
		 * height = heights + (STATE_LABEL_PAD << 1); // Upper corner of the box. int x
		 * = point.x - (width >> 1); int y = point.y + STATE_RADIUS - STATE_LABEL_PAD;
		 * // Where the y point of the baseline is. int baseline = y;
		 * 
		 * g.setColor(color); g.fillRect(x, y, width, height); g.setColor(Color.black);
		 * for (int i = 0; i < labels.length; i++) { baseline += ascent +
		 * STATE_LABEL_PAD; g.drawString(labels[i], x + STATE_LABEL_PAD, baseline); }
	 ...
	 * @param bool
	 */
	public void drawBackground(Graphics g, TextLabel textLabel, Point point, Color color) {
		g.setColor(color);
...
//		}
	...
	public static Color STATE_COLOR = new Color(255, 25, 150);

	/** The color of a selected state. **/
	public static Color HIGHLIGHT_COLOR = new Color(100, 200, 100);
